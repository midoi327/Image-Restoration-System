...
def main():
    ...
    input_path = os.path.join('demo', 'crop_before') # Set12 폴더를 input으로 사용 
    output_path = os.path.join('demo', 'crop_after') # Output 폴더에 denoising 이미지 저장 

    files_source = glob.glob(os.path.join('demo','crop_before', '*.png')) # 테스트하려는 이미지 
    files_source.sort()
